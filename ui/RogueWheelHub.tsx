...
export default function RogueWheelHub(props: HubShellProps) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setSelected((i) => wrapIndex(i + 1, items.length));
    ...
    <div className="relative min-h-screen w-full overflow-hidden text-white">
      ...
      <div className="px-6 pt-10 md:px-10 max-w-xl">
        ...
        <div className="mt-2 flex items-center justify-start gap-4">
          <p className="text-purple-100/90 md:text-lg">
            ...
            <li key={it.key} className="mb-3">
              <button
                ...
                className={[
                  ...
                  i === selected
                    ? "bg-gradient-to-r from-amber-300 to-amber-500 text-indigo-950 shadow-[0_6px_18px_rgba(255,191,71,0.35)] ring-2 ring-amber-300 before:absolute before:left-0 before:top-1/2 before:h-4 before:w-4 before:-translate-x-1/2 before:-translate-y-1/2 before:rotate-45 before:bg-amber-300"
      ...
      <footer className="pointer-events-none absolute bottom-0 left-0 right-0 flex flex-wrap items-end justify-end gap-2 px-6 pb-4 text-sm opacity-85 md:px-10">
        <div className="pointer-events-auto rounded bg-black/35 px-3 py-1.5 ring-1 ring-amber-300/25">
          {version}
        </div>
      </footer>
  ...
  return (
    <div role="dialog" aria-modal className="fixed inset-0 z-40 grid place-items-center">
      ...
      <div className="relative z-10 w-[92vw] max-w-2xl rounded-2xl bg-indigo-950/90 p-4 ring-1 ring-amber-300/25 backdrop-blur md:p-6">
        ...
        <div className="max-h-[70vh] overflow-auto pr-1 text-sm leading-6 text-white/90">
          {children}
        </div>
      </div>
    </div>
...
function HowToContent() {
  return (
    <div className="space-y-4">
      <p>
        <b>Goal:</b> Win rounds by earning victories on the three wheels.
      </p>
      <ol className="list-decimal pl-5 space-y-2">
        <li>
          <b>Draft:</b> Each round everyone draws <b>5 cards</b>.
        </li>
        <li>
          <b>Commit:</b> Select and place down <b>3</b> of your cards, <b>1</b> beside each wheel. The remaining <b>2</b> go to your
          <b> Reserve</b>.
        </li>
        <li>
          <b>Spin:</b> The token on each wheel moves equal to the <b>sum of the cards placed beside it</b>.
        </li>
        <li>
          <b>Resolve:</b> The landing section of each wheel determines the winner for that wheel:
          <ul className="mt-1 list-disc pl-5">
            <li>
              <b>Largest Number</b> – higher committed number wins.
            </li>
            <li>
              <b>Biggest Reserve</b> – higher total of reserve cards wins.
            </li>
            <li>
              <b>Smallest Card</b> – lower committed number wins.
            </li>
            <li>
              <b>Initiative</b> – initiative holder wins ties here.
            </li>
          </ul>
        </li>
        <li>
          <b>Advance:</b> Winners are counted, then a new round begins.
        </li>
      </ol>
      ...
      <div className="grid gap-3 rounded-xl bg-white/5 p-3 ring-1 ring-white/10">
        ...
        <ul className="list-disc pl-5 space-y-1">
          ...
          <li>
            Use your Reserve to build advantage for <i>Biggest Reserve</i>.
          </li>
          <li>
            Track <b>initiative</b>: It breaks ties on sections.
          </li>
        </ul>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="rounded-xl bg-white/5 p-3 ring-1 ring-white/10">
          ...
          <ul className="mt-1 list-disc pl-5">
            ...
            <li>
              Mouse/touch: Tap and hold to select a card, then tap on a space to place it.
            </li>
          </ul>
        </div>
        <div className="rounded-xl bg-white/5 p-3 ring-1 ring-white/10">
    ...
    >
      <Field label="Music Volume">
        <input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            setMusic(parseFloat(e.target.value))
          }
          className="w-full"
        />
      </Field>
      <Field label="SFX Volume">
        <input
          ...
          value={sfx}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
            setSfx(parseFloat(e.target.value))
          }
          className="w-full"
        />
      </Field>
      ...
      <div className="flex justify-end gap-2 pt-2">
        <button
          type="button"
          className="rounded-xl bg-white/10 px-3 py-1.5 ring-1 ring-white/15 hover:bg-white/15"
        >
          Cancel
        </button>
        <button
          type="submit"
          className="rounded-xl bg-amber-400 px-3 py-1.5 font-semibold text-indigo-950 ring-1 ring-amber-300 hover:brightness-95"
        >
          Apply
        </button>
      </div>
...
(function __runtime_tests__() {
  try {
    ...
    console.assert(
      itemsNoSave[0].label === "Play",
      "First item should be Play when no save"
    );

    const itemsWithSave = buildMenuItems({ hasSave: true });
    console.assert(
      itemsWithSave[0].label === "Continue",
      "First item should be Continue when save exists"
    );

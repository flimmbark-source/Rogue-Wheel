...
export default function ThreeWheel_WinsOnly() {
  ...
  const renderWheelPanel = (i: number) => {
    ...
    return (
      <div
        className="relative rounded-lg border p-0 shadow flex-none"
        style={{
          width: panelW,
          height: ws,
      ...
      >
        ...
        <div className="flex items-center justify-center gap-1" style={{ height: ws - 2 }}>
          {/* Player slot */}
          <div
          ...
          {/* Wheel face */}
          <div className="relative" onDragOver={onZoneDragOver} onDragEnter={onZoneDragOver} onDragLeave={onZoneLeave} onDrop={onZoneDrop} onClick={(e) => { e.stopPropagation(); tapAssignIfSelected(); }} aria-label={`Wheel ${i+1}`}>
          ...
          {/* Enemy slot */}
          <div className="w-[80px] h-[92px] rounded-md border px-1 py-0 flex items-center justify-center" style={{ backgroundColor: THEME.slotBg, borderColor: THEME.slotBorder }} aria-label={`Wheel ${i+1} enemy slot`}>

...
type Props = { sections: Section[]; size: number; onTapAssign?: () => void; };

const CanvasWheel = memo(forwardRef<WheelHandle, Props>(({ sections, size, onTapAssign }, ref) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const tokenElRef = useRef<HTMLDivElement | null>(null);
  const tokenSliceRef = useRef<number>(0);

  const CLIP_PAD = 3;
  const WHEEL_OFFSET_X = -8;
  ...
  useEffect(() => { drawBase(); /* eslint-disable-line */ }, [size, sections]);
  ...
  return (
    <div onClick={onTapAssign} className="relative overflow-hidden rounded-full" style={{ width: size, height: size, contain:'paint', transform:'translateZ(0)', backfaceVisibility:'hidden' }}>
      <canvas ref={canvasRef} aria-hidden style={{ position: 'absolute', inset: 0, display: 'block' }} />
      <div ref={tokenElRef} aria-hidden style={{ position:'absolute', width:14, height:14, left:0, top:0, borderRadius:9999, background:'#fff', border:'2px solid #0f172a', willChange:'transform' }} />
    </div>
  );
}));
